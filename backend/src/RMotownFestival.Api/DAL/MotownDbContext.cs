...
namespace RMotownFestival.Api.DAL
{
    public class MotownDbContext : DbContext
    {
        ...
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            ...
            modelBuilder.Entity<Schedule>().HasData(lineUp);
            modelBuilder.Entity<ScheduleItem>().HasData(new ScheduleItem { Id = 1, ScheduleId = lineUp.Id, ArtistId = artists[0].Id, StageId = stages[1].Id, Time = new DateTime(1972, 07, 01, 20, 30, 0) });
            modelBuilder.Entity<ScheduleItem>().HasData(new ScheduleItem { Id = 3, ScheduleId = lineUp.Id, ArtistId = artists[2].Id, StageId = stages[0].Id, Time = new DateTime(1972, 07, 01, 22, 0, 0) });
            modelBuilder.Entity<ScheduleItem>().HasData(new ScheduleItem { Id = 4, ScheduleId = lineUp.Id, ArtistId = artists[1].Id, StageId = stages[1].Id, Time = new DateTime(1972, 07, 01, 22, 15, 0) });
            modelBuilder.Entity<ScheduleItem>().HasData(new ScheduleItem { Id = 5, ScheduleId = lineUp.Id, ArtistId = artists[0].Id, StageId = stages[0].Id, Time = new DateTime(1972, 07, 02, 20, 15, 0) });
        }
    }
}
